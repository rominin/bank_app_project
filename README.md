...
4) Есть возможность собирать сразу все микросервисы одной командой (например, с помощью parent POM и использования мультимодульных проектов).
5) Код приложения написан на Java 21.
6) Web UI приложения использует Spring Web MVC.
7) (TODO решить: JDBC vs JPA | склоняюсь к JPA) Приложение может использовать Spring Data JDBC или Spring Data JPA и Hibernate ORM под капотом для доступа к данным в базе данных.
8) Базы данных для микросервисов приложения персистентные (PostgreSQL). База данных одна, но микросервисы разграничены на уровне схем, чтобы реализовать паттерн Database per Service.
9) (TODO решить: Consul vs Eureka vs самописный сервер | склоняюсь к Consul) В качестве Service Discovery и Gateway API можно применять Consul, Eureka или самописный сервис, реализующий паттерны Service Discovery и Gateway API, с использованием проекта Spring Cloud Gateway.
10) Микросервисы выполняют запросы в другие микросервисы через Gateway API и регистрируются в Service Discovery.
11) (TODO решить: Consul vs Zookeeper vs самописный сервер | склоняюсь к Consul) В качестве Externalized/Distributed Config можно применять Consul, ZooKeeper или самописный сервис, реализующий паттерн Externalized/Distributed Config, с использованием проекта Spring Cloud Config.
...
15) (TODO решить: Ribbon+Hystrix vs Resilience4J vs etc) Микросервисы должны уметь выполнять ретраи запросов, а также иметь фоллбэки на эндпоинты заглушки (это могут быть Ribbon и Hystrix, Resilence4j и т. д. на ваш выбор).
